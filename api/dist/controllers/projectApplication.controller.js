...
class ProjectApplicationController {
    ...
    async submitApplication(req, res) {
        try {
            ...
            const { coverLetter, proposedBudget, estimatedDeliveryDays, portfolioLinks, } = req.body;
            ...
            if (error.message.includes('not accepting applications') ||
     ...
     * @access  Private (Influencer)
     */
    async getInfluencerApplications(req, res) {
        try {
            const { role, influencer } = req.user;
            if (role !== 'INFLUENCER' || !influencer) {
                return res.status(403).json({
            ...
            const result = await projectApplicationService.getInfluencerApplications(influencer.id, status, pageNum, limitNum);
    ...
    async getProjectApplications(req, res) {
        try {
            ...
            const applications = await projectApplicationService.getProjectApplications(projectId, salon.id, status);
            return res.status(200).json({
                success: true,
                data: applications,
            });
    ...
    async rejectApplication(req, res) {
        try {
            ...
            const result = await projectApplicationService.rejectApplication(id, salon.id, reason?.trim());
            return res.status(200).json({
                success: true,
                data: result.application,
                message: result.message,
    ...
    async withdrawApplication(req, res) {
        try {
            ...
            if (error.message.includes('do not own this application') ||
                error.message.includes('Only PENDING applications')) {
                return res.status(400).json({
                    error: 'Bad Request',
                    message: error.message,
