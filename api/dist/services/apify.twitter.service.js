...
class ApifyTwitterService {
        ...
        try {
            ...
            try {
                ...
                // Structure 4: User data with screen_name/username
                    ...
                    tweetsArray = items.slice(1).filter((item) => item.id_str || item.id);
     ...
     */
    async connectTwitterAccount(userId, username) {
        try {
     ...
     */
    async storeTweets(accountId, tweets) {
        try {
            // Delete old tweets
     ...
     */
    async syncTwitterData(accountId) {
        try {
    ...
    async getTwitterData(accountId) {
        try {
            ...
            const recentTweets = account.posts.map(post => ({
                ...
                mediaUrls: post.metadata?.mediaUrls || [],
                ...
                quoteCount: post.metadata?.quoteCount || 0,
            }));
     ...
     */
    async disconnectTwitterAccount(accountId, userId) {
        try {
            // Verify ownership
