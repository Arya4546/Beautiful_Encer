 ...
 * 
 * Uses Apify's TikTok Scraper actor to fetch TikTok data
...
class ApifyTikTokService {
  ...
  private readonly CACHE_DURATION_DAYS = 7; // Cache data for 7 days (same as Instagram)
   ...
   */
  private calculateEngagementRate(videos: TikTokPublicVideo[], followersCount: number): number {
   ...
   */
  private extractTopHashtags(videos: TikTokPublicVideo[]): string[] {
   ...
   */
  private normalizeProfileData(rawData: any): TikTokPublicProfile | null {
    ...
    const profile: TikTokPublicProfile = {
      username: username,
   ...
   */
  private normalizeVideoData(rawVideo: any): TikTokPublicVideo | null {
   ...
   */
  async scrapeTikTokProfile(username: string): Promise<ScrapedTikTokData> {
    try {
      ...
      const recentVideos: TikTokPublicVideo[] = [];
      ...
      const scrapedData: ScrapedTikTokData = {
        username: profile.username,
        displayName: profile.displayName,
   ...
   */
  async connectTikTokAccount(userId: string, username: string): Promise<any> {
    try {
      // Remove @ if user included it
      const cleanUsername = username.replace('@', '').trim();
   ...
   */
  async syncTikTokData(accountId: string): Promise<any> {
    try {
   ...
   * @param accountId - Social media account ID
   */
  async disconnectTikTokAccount(accountId: string): Promise<void> {
    try {
      const account = await prisma.socialMediaAccount.findUnique({
        where: { id: accountId },
      });
   ...
   */
  async scrape(username: string): Promise<TikTokScrapeResult> {
   ...
   */
  async connectPublicAccount(userId: string, username: string) {
