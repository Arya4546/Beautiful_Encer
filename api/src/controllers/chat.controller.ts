...
class ChatController {
  ...
  async getContacts(req: any, res: Response) {
    try {
      ...
      const { search = '', role, cursor, limit = '20' } = req.query as Record<string, string>;
  ...
  const requests = await prisma.connectionRequest.findMany(queryArgs as any);

      // Map to other users and apply search/role filters in-memory for simplicity
      const contactsRaw = (requests as any[]).map((req) => {
      ...
      let filtered = contactsRaw.filter((c) => {
        if (role && c.otherUser.role !== role) return false;
