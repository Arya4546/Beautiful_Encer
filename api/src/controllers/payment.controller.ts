...
class PaymentController {
        ...
        switch (event.type) {
          ...
          case 'invoice.payment_succeeded': {
            ...
            if ((invoice as any).subscription) {
              try {
                const subscription = await (await import('../config/stripe.config.js')).stripe.subscriptions.retrieve((invoice as any).subscription as string);
                await paymentService.handleSubscriptionUpdate(subscription);
              } catch (error: any) {
   ...
   * @access  Public
   */
  async syncSalonPayment(req: Request, res: Response) {
    try {
      const { salonId } = req.params;
      ...
      let periodStart = (subscription as any).current_period_start || (subscription as any).billing_cycle_anchor || (subscription as any).created;
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (!existingPayment) {
          await tx.payment.create({
            data: {
              stripePaymentId: subscription.latest_invoice as string || `manual_${Date.now()}`,
