...
class ProjectService {
    ...
    async createProject(params) {
        try {
            const { salonId, influencerId, title, projectType, description, startDate, endDate, budget, deliverables, requirements, location, category, } = params;
            // Validate inputs
    ...
    async getProjectById(projectId, userId) {
        try {
            ...
            const isAuthorized = (user.salon && user.salon.id === project.salonId) ||
    ...
    async cancelProject(projectId, userId) {
        try {
            ...
            const allowedStatuses = [ProjectStatus.PENDING, ProjectStatus.ACCEPTED];
     ...
     */
    async startProject(projectId, userId) {
        try {
            if (!projectId) {
                throw new Error('Project ID is required');
            }
     ...
     */
    async getSalonProjectStats(salonId, userId) {
        try {
     ...
     */
    async getInfluencerProjectStats(influencerId, userId) {
        try {
