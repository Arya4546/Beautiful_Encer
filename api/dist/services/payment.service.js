...
class PaymentService {
    ...
    async handleSuccessfulPayment(session) {
        try {
            ...
            if (!periodStart || !periodEnd) {
                ...
                const billingCycleAnchor = stripeSubscription.billing_cycle_anchor || stripeSubscription.created;
            ...
            await prisma.$transaction(async (tx) => {
                ...
                await tx.payment.create({
                    data: {
                        stripePaymentId: session.payment_intent || `pi_${Date.now()}`,
                        stripeSessionId: session.id,
                ...
                if (salonWithUser?.user?.email) {
                    ...
                    sendSubscriptionPaymentEmail(salonWithUser.user.email, salonWithUser.businessName || salonWithUser.user.name, planName, amount, formatDateJapanese(new Date()), formatDateJapanese(currentPeriodEnd));
                }
            });
     ...
     */
    async getSalonSubscriptionStatus(salonId) {
        try {
            // Validate input
            if (!salonId) {
