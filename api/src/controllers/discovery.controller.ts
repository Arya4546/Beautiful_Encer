...
class DiscoveryController {
   ...
   */
  async getInfluencers(req: AuthenticatedRequest, res: Response) {
    try {
   ...
   */
  async getSalons(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.userId;
      ...
      // Parse query parameters
      const { page = '1', limit = '12', search, categories } = req.query as DiscoveryQuery;
      const pageNum = parseInt(page, 10);
      const limitNum = parseInt(limit, 10);
      const skip = (pageNum - 1) * limitNum;
   ...
   */
  async getRegions(req: AuthenticatedRequest, res: Response) {
    try {
