...
class ProjectMarketplaceController {
  ...
  async getPublicProjects(req: Request, res: Response) {
    try {
      ...
      const result = await projectMarketplaceService.getPublicProjects(
        filters,
        pageNum,
        limitNum
      );
      ...
      const projectsWithAppStatus = await Promise.all(
        result.projects.map(async (project: any) => {
          if (!influencer?.id) return project;
  ...
  async getProjectById(req: Request, res: Response) {
    try {
      ...
      if (role === 'INFLUENCER' && influencer && result.project.applications) {
        const application = result.project.applications.find(
          (app: any) => app.influencerId === influencer.id
        );
        if (application) {
          hasApplied = true;
   ...
   * @access  Private (Salon)
   */
  async getSalonProjects(req: Request, res: Response) {
    try {
      ...
      const result = await projectMarketplaceService.getSalonProjects(
        salon.id,
        status as any,
        pageNum,
        limitNum
   ...
   * @access  Private (Salon)
   */
  async closeApplications(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const { role, salon } = req.user!;
