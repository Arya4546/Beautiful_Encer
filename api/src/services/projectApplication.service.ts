...
class ProjectApplicationService {
   ...
   */
  async submitApplication(data: CreateApplicationDto) {
    try {
   ...
   */
  async getInfluencerApplications(influencerId: string, status?: ApplicationStatus, page: number = 1, limit: number = 10) {
    try {
   ...
   */
  async getProjectApplications(projectId: string, salonId: string, status?: ApplicationStatus) {
    try {
   ...
   */
  async acceptApplication(applicationId: string, salonId: string) {
    try {
   ...
   */
  async rejectApplication(applicationId: string, salonId: string, rejectionReason?: string) {
    try {
      const application = await prisma.projectApplication.findUnique({
        where: { id: applicationId },
        include: {
   ...
   */
  async withdrawApplication(applicationId: string, influencerId: string) {
    try {
      const application = await prisma.projectApplication.findUnique({
        where: { id: applicationId },
        include: {
   ...
   */
  async hasApplied(projectId: string, influencerId: string) {
    try {
      const application = await prisma.projectApplication.findUnique({
        where: {
